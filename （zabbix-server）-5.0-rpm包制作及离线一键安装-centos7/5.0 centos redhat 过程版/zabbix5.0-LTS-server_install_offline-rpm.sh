#!/bin/bash
clear
echo -e "\033[33m--------------------------zabbix5.0.17-server-安装条件-------------------
...
------------------------------------------安装方法-----------------------
1.请先将做好的zabbix5.0_LTS-os$OS-rpm.tar.gz上传到此服务器的/root下
2.运行脚本sh zabbix5.0-LTS-server_install_offline(rpm).sh
...
echo "第二步:将rpm压缩包解压"
tar zxvf /root/zabbix5.0_LTS-os"$OS"-rpm.tar.gz
cd zabbix5.0_LTS-os"$OS"-rpm
echo "第三步：创建相关的用户，并依次安装相关服务"
groupadd zabbix && useradd -g zabbix zabbix
groupadd nginx && useradd -g nginx nginx
groupadd mysql && useradd -g mysql mysql
groupadd apache && useradd -g apache apache
rpm -ivh  au* c* f* g* lib* O* p* s* u* mariadb-lib* net* mariadb* --nodeps --force
rpm -ivh  r* zabbix-nginx* zabbix-web* zabbix-agent* zabbix-server*  --nodeps --force
